...
| The catalog lists macros that fail to run (never even reach `pending`) right after a **cluster** recreate (not just a pod restart) | The macro registry (`registry.db`, a local SQLite file) and each macro's *built Docker image* are two separate things. The registry survives a cluster recreate — it's just a file on your machine — but the images only ever lived inside the old cluster's containerd store, and a full `k3d cluster delete` throws that away completely. The catalog looks populated but every image behind it is gone. | Rebuild each macro that needs to run again — open it in the catalog and save/rebuild (`POST /macros/{name}/build`), which re-runs the real `docker build` + `k3d image import` into the current cluster. Confirm with `docker exec k3d-radio-maas-server-0 crictl images \| grep <macro_name>` if unsure whether a given macro's image actually exists in the current cluster. |
